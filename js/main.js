    ...
    if (mobileMenu && !document.getElementById('mobile-menu-footer')) {
        ...
        footerDiv.innerHTML = `
            ...
            <div style="display: flex; align-items: flex-start; gap: 8px; margin-bottom: 4px;">
                <span class="material-symbols-outlined" style="font-size: 14px; color: #C5A059; margin-top: 2px;">call</span>
                <div style="display: flex; flex-direction: column; gap: 2px;">
                    <span>+221 33 843 15 25</span>
                    <span>+221 78 681 15 26</span>
                </div>
            </div>
...
Téléphone Fixe   : +221 33 843 15 25
Téléphone Mobile : +221 78 681 15 26
...
function downloadTicketImage(data) {
    ...
    tempCtx.fillText('Ouest Foire, Dakar, Sénégal', tempCanvas.width / 2, 550);
    tempCtx.fillText('Tél Fixe: +221 33 843 15 25  |  Mobile: +221 78 681 15 26', tempCanvas.width / 2, 570);
...
function initChatDrawer(drawer) {
    ...
    function checkApiKeyStatus() {
        ...
        if (!key) {
            if (isAdmin) {
                ...
                userInput.placeholder = "Mode Démo : Configurez la clé API...";
            } else {
                apiWarning.style.display = 'none';
                userInput.placeholder = "Posez votre question (Antoine)...";
            }
    ...
    // Helper for local simulation when API key is not configured
    function getSimulatedReply(userQuery) {
        ...
        // 8. Thank you
        if (text.includes("merci") || text.includes("thanks") || text.includes("remercie") || text.includes("gentil") || text.includes("parfait") || text.includes("super")) {
            return "C'est un véritable plaisir de vous assister ! N'hésitez pas si vous avez besoin d'autres conseils pour sublimer votre regard.";
        }
