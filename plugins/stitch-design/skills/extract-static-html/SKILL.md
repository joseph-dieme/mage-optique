...
| **Fidelity** | **Highest â€” computed styles resolved** | High â€” rendered DOM |
| **Setup** | **Zero â€” no mock needed** | Zero â€” no mock needed |
| **Framework** | **Any** | Any |
| **Output** | **Writes to file â€” no size limit** | May truncate in agent context |

> [!WARNING]
> **Checkpoint â€” User Confirmation Required.**
...
Launches headless Chrome, captures the fully rendered DOM, and produces a self-contained HTML file with all CSS inlined and images as base64. Works with **any framework** â€” no MockPage.jsx needed.
...
3.  **Multiple pages** â€” run once per route:
    ```bash
    npx tsx <SKILL_DIR>/scripts/snapshot.ts \
...
| `--html-class` | â€” | Class(es) for `<html>` element (e.g., `dark`) |
...
| `--title` | â€” | Override page title |

### What It Does Automatically

- Inlines all `<link rel="stylesheet">` â†’ `<style>` blocks
- Converts `<img>` `src` **and `srcset`** â†’ base64 data URIs (skips fonts)
...
| Images show as broken after server stops | Verify `srcset` was inlined â€” check log for "Inlined N images". If `srcset` URLs failed, they are auto-removed so `src` (inlined) is used. |
| Next.js `/_next/image` not inlined | Ensure the dev server is running when snapshot runs â€” the script fetches optimized images from the running server. |
...
1. **Include the full layout** â€” header, sidebar, footer (read `App.js` first)
2. **Flatten all conditionals** â€” pick one state, remove all ternaries and `&&` guards
3. **Hardcode all data** â€” replace `{variable}` with concrete values, unroll `.map()` loops
4. **Preserve logos** â€” use `<img>` with local paths (post-process will inline them)
5. **Remove floating elements** â€” cookie banners, chat widgets, feedback buttons
