...
## ðŸŽ¨ Prompt Enhancement Pipeline
...
- **Design System**: Check if a design system exists for the project via
  ...
  are already applied at the project level â€” do NOT include any color, font, or
...
and structure** â€” never include colors, fonts, or theme instructions (these are
...
- User wants to create from a text description â†’ **Generate from Text** flow
- User provides an image, screenshot, or mockup â†’ **Generate from Image** flow
- User wants to modify an existing screen â†’ **Edit** flow
- User wants layout/color/content variations â†’ **Generate Variants** flow
...
### Generate from Image Flow (Image/Mockup â†’ Design)
...
> "make it look like this". For example: "This is a dashboard mockup â€” recreate
...
**Variant Options:**
- **`variantCount`**: 1â€“5 variants (default: 3)
- **`creativeRange`**: `REFINE` (subtle), `EXPLORE` (balanced), or `REIMAGINE`
  (radical)
- **`aspects`**: Focus on specific dimensions â€” `LAYOUT`, `COLOR_SCHEME`,
...
- **No theme leakage**: Never put hex codes, font names, or color roles in a
  generation prompt â€” the design system handles all visual styling.
...
- [Design Mappings](references/design-mappings.md) â€” UI/UX keywords and
  atmosphere descriptors.
- [Prompting Keywords](references/prompt-keywords.md) â€” Technical terms Stitch
  understands best.
- [Enhanced Prompt Example](examples/enhanced-prompt.md) â€” Before/after prompt
  enhancement.
