...
description: >-
  ...
  code â€” React, Vue, Svelte, Angular, plain HTML/CSS, or any web framework. Analyzes
  ...
  understand the styling patterns in a frontend repo â€” even if they just say "what
...
(DESIGN.md) that captures the project's visual language â€” colors, typography,
spacing, component patterns, and layout principles â€” directly from the source
...
and want to understand its design system before you can even run the app â€”
...
- No build or runtime dependencies needed â€” this skill reads source files only
...
Read `package.json` first â€” it reveals the framework, CSS tooling, and any
...
```
src/
â”œâ”€â”€ components/     â† Component-level styles
â”œâ”€â”€ styles/         â† Global stylesheets
â”œâ”€â”€ theme/          â† Theme definitions, tokens
â”œâ”€â”€ assets/         â† Fonts, images
â”œâ”€â”€ app.css         â† Root styles
â””â”€â”€ index.css       â† Entry CSS
```

Also check for:
- `tailwind.config.js` / `tailwind.config.ts` â€” Custom colors, fonts, spacing
- `globals.css` / `global.css` â€” CSS custom properties (variables)
...
- **React / Next.js / Tailwind** â†’ [references/react-tailwind.md](references/react-tailwind.md)
- **Vue / Nuxt** â†’ [references/vue.md](references/vue.md)
- **Svelte / SvelteKit** â†’ [references/svelte.md](references/svelte.md)
- **Angular** â†’ [references/angular.md](references/angular.md)
- **Plain CSS / SASS / Less** â†’ [references/plain-css.md](references/plain-css.md)
...
The goal isn't to dump every CSS property â€” it's to understand the *intent*
...
1. **Primary Foundation** â€” Background and surface colors
2. **Accent & Interactive** â€” CTA buttons, active states, links
3. **Typography & Text Hierarchy** â€” Primary, secondary, tertiary text
4. **Functional States** â€” Success, error, warning, info

For each color, create a descriptive name that evokes the color's character
rather than its raw hex value:

- âŒ `#294056` â†’ "Blue"
- âœ… `#294056` â†’ **"Deep Muted Teal-Navy"** â€” Primary CTA, active navigation
...
- Find every heading level (H1-H6) and body text, noting:
  ...
  - `letter-spacing` (and why â€” elegance? compactness?)
...
**Buttons:**
- Corner radius (and what it communicates â€” playful? professional? minimal?)
...
**Cards / Containers:**
- Corner radius (often different from buttons â€” slightly rounder)
...
2. The DESIGN.md you wrote is the input â€” the manage-design-system skill handles
...
- **Read comments and commit messages.** Developers often document design
  intent in code comments (`/* hero section â€” breathable */`) and commit
...
- **Tailwind config is a design system.** If a project has a customized
  `tailwind.config.js`, that *is* the design system â€” extract from it first,
