...
1. **`src/app.css` / `src/app.postcss`** â€” Global styles and CSS custom
   properties. The most important file.

2. **`svelte.config.js`** â€” May reference CSS preprocessors, Tailwind, or
   UnoCSS configuration.

3. **`tailwind.config.js`** (if Tailwind/UnoCSS) â€” Same extraction as React.

4. **`src/lib/theme.ts` / `src/lib/tokens.ts`** â€” Shared design tokens
   exported as JS objects.

5. **`src/routes/+layout.svelte`** â€” Root layout. Shows global font loading,
   background, and structural patterns.

6. **Component `<style>` blocks** â€” Scoped styles revealing component patterns.
...
- `var(--*)` references â†’ trace to `app.css`
- Transition values reveal the interaction design philosophy

## SvelteKit Layout Patterns

- **`+layout.svelte`** at route root â€” Global header, footer, font loading
- **`+layout.ts/js`** â€” May load theme data or tokens
- **`$lib/`** directory â€” Reusable components and shared utilities
...
- **DaisyUI**: Theme in `tailwind.config.js` â†’ `daisyui.themes` array.
