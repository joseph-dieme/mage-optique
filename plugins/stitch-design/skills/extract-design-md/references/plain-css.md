...
For projects without a JavaScript framework â€” static sites, WordPress
themes, vanilla HTML/CSS, or CSS preprocessor-heavy projects.

## File Discovery Order

1. **`index.html` / `*.html`** â€” Check `<link>` tags and `<style>` blocks
   for stylesheet references, inline styles, and font loading.

2. **Main stylesheet** (`style.css`, `main.css`, `app.css`) â€” The primary
   CSS file. Look for custom properties, base styles, and typography.

3. **`_variables.scss` / `_tokens.scss` / `variables.less`** â€” Preprocessor
   variable files containing design tokens.

4. **`_mixins.scss`** â€” Reusable style patterns reveal design conventions.

5. **Component/module stylesheets** â€” Individual CSS files for UI components.
...
Same extraction approach â€” map each variable to a descriptive name and role.
