...
1. **`nuxt.config.ts` / `nuxt.config.js`** â€” May contain global CSS paths,
   font configuration, and Tailwind/UnoCSS module config.

2. **`assets/css/main.css`** (or similar) â€” Global styles, CSS custom
   properties, and font imports.

3. **`tailwind.config.js`** (if Tailwind is used) â€” Same as React; extract
   custom theme values directly.

4. **`plugins/vuetify.ts`** (if Vuetify) â€” Theme definition with custom
   palette and typography.

5. **Component `<style>` blocks** â€” Co-located styles (scoped or global).
...
- `var(--*)` references â†’ trace back to global CSS for the actual values
- `scoped` styles â†’ component-specific, but reveal consistent patterns
- BEM naming (`.card__title`) â†’ hints at component hierarchy
...
- **Quasar**: `quasar.config.js` â†’ `framework.config.brand` for colors
- **PrimeVue**: CSS themes in `assets/` or theme preset configuration
- **Element Plus**: SCSS variables in `element-variables.scss`

Look for the override file â€” that's where the project's unique values live.
...
- **`app.vue`** or **`layouts/default.vue`** â€” Root layout, reveals
  global background, font loading, and overall structure.
- **`assets/`** â€” Global CSS, fonts, and images.
- **`composables/`** â€” May contain `useTheme` or `useDesignTokens`.
- **`nuxt.config.ts`** `css` array â€” Lists global stylesheets automatically
