...
Read these files in priority order â€” higher-priority files give you the
intended design system, lower-priority files show what actually shipped:

1. **`tailwind.config.js` / `tailwind.config.ts`** â€” The single most
...
3. **`theme.ts` / `theme.js` / `tokens.ts`** â€” Explicit design token files.
   May export objects consumed by Tailwind config or CSS-in-JS providers.

4. **`src/app/layout.tsx` or `src/App.tsx`** â€” Root layout. Shows the global
   font setup (via `next/font` or `<link>`), body background, and overall
   structure.

5. **Component files (`*.tsx` / `*.jsx`)** â€” Look at 5-8 representative
...
module.exports = {
  theme: {
    extend: {
      colors: {
        primary: '#294056',      // â†’ "Deep Muted Teal-Navy" â€” Primary CTA
        background: '#FCFAFA',   // â†’ "Warm Barely-There Cream" â€” Page BG
      },
      fontFamily: {
...
These are explicitly declared tokens â€” use their names as clues for
their intended role.
...
- **Chakra**: Look for `extendTheme()` calls â€” these override defaults.
- **MUI**: Look for `createTheme()` â€” palette, typography, and spacing overrides.
...
The overrides are the design system â€” default values are the library's generic
