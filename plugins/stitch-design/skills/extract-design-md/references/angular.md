...
1. **`angular.json`** â€” Lists global style files under
   `projects.*.architect.build.options.styles`. These are the entry-point
   CSS/SCSS files.

2. **`src/styles.scss` / `src/styles.css`** â€” Global stylesheet. CSS custom
   properties, font imports, and base styles live here.

3. **`src/theme.scss` / `src/theme/`** â€” Explicit theme directory. Custom
   Material/component palettes.

4. **`tailwind.config.js`** (if Tailwind) â€” Same extraction as React.

5. **`src/app/app.component.scss`** â€” Root component styles, reveals global
   layout patterns.

6. **Component `.scss` / `.css` files** â€” Co-located styles (ViewEncapsulation
   scoped by default).
...
**What to extract:**
- Palette choices â†’ map to functional color roles
- Typography config â†’ maps directly to the hierarchy section
- Light vs dark theme â†’ atmosphere
...
- Look for `:host` selectors â€” these style the component's root element
- `::ng-deep` (deprecated but still used) â€” styles that pierce encapsulation
...
- **PrimeNG**: Theme SCSS in `node_modules/primeng/resources/themes/` â€”
