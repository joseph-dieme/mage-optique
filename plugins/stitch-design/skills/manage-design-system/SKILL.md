...
## ðŸš€ Create or Update Design System in Stitch
...
Stitch holds the design tokens at the project level â€” you do NOT need to repeat
them in generation prompts.

## ðŸŽ¨ Apply Design System to Screens

Use `apply_design_system` to apply a design system to existing screens.

> [!IMPORTANT]
> `selectedScreenInstances` must contain **only** `id` and `sourceScreen` â€” do
...
1. Call `get_project` to retrieve `screenInstances` â€” each has an `id` and
   `sourceScreen`.
2. Call `list_design_systems` to retrieve the design system `name` (format:
   `assets/{assetId}`) â€” use the part after `assets/` as the `assetId`.
3. Filter out any instances with `type: "DESIGN_SYSTEM_INSTANCE"` â€” only pass
   real screens.
