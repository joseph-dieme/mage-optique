...
**Optional:**
- Chrome DevTools MCP Server â€” enables visual verification of generated pages
...
**Important checks:**
- Section 4 (Sitemap) â€” Do NOT recreate pages that already exist
- Section 5 (Roadmap) â€” Pick tasks from here if backlog exists
- Section 6 (Creative Freedom) â€” Ideas for new pages if roadmap is empty
...
4. **Retrieve assets**: Before downloading, check if `.stitch/designs/{page}.html` and `.stitch/designs/{page}.png` already exist:
   ...
   - **If files do not exist**: Proceed with download:
     - `htmlCode.downloadUrl` â€” Download and save as `.stitch/designs/{page}.html`
      - `screenshot.downloadUrl` â€” Append `=w{width}` to the URL before downloading, where `{width}` is the `width` value from the screen metadata (Google CDN serves low-res thumbnails by default). Save as `.stitch/designs/{page}.png`
...
project/
â”œâ”€â”€ .stitch/
â”‚   â”œâ”€â”€ metadata.json   # Stitch project & screen IDs (persist this!)
â”‚   â”œâ”€â”€ DESIGN.md       # Visual design system (from design-md skill)
â”‚   â”œâ”€â”€ SITE.md         # Site vision, sitemap, roadmap
â”‚   â”œâ”€â”€ next-prompt.md  # The baton â€” current task
â”‚   â””â”€â”€ designs/        # Staging area for Stitch output
â”‚       â”œâ”€â”€ {page}.html
â”‚       â””â”€â”€ {page}.png
...
| `screens` | Map of page name â†’ screen object. Each screen includes `id`, `sourceScreen` (resource path for MCP calls), canvas position (`x`, `y`), and dimensions (`width`, `height`) |
...
The skill is orchestration-agnostic â€” focus on the pattern, not the trigger mechanism.
...
- âŒ Forgetting to update `.stitch/next-prompt.md` (breaks the loop)
- âŒ Recreating a page that already exists in the sitemap
- âŒ Not including the design system block from `.stitch/DESIGN.md` in the prompt
- âŒ Leaving placeholder links (`href="#"`) instead of wiring real navigation
- âŒ Forgetting to persist `.stitch/metadata.json` after creating a new project
