...
async function validateComponent(filePath) {
  ...
  try {
    ...
    if (hasInterface) {
      console.log("âœ… Props declaration found.");
    } else {
      console.error("âŒ MISSING: Props interface (must end in 'Props').");
    }

    if (tailwindIssues.length === 0) {
      console.log("âœ… No hardcoded hex values found.");
    } else {
      console.error(`âŒ STYLE: Found ${tailwindIssues.length} hardcoded hex codes.`);
    ...
    console.error("âŒ PARSE ERROR:", err.message);
    process.exit(1);
  }
}
